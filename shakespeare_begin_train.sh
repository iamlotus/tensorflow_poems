...
if [ -f ".sptrainpid" ]; then
    if [ -d /proc/`cat .sptrainpid` ]; then
        ...
        && nohup python3 train.py --cuda_visible_devices=1 --input_name=shakespeare --learning_rate=0.001 --epochs=1000 --input_name=poems >logs/sptrain.out 2>&1 & echo $! > .sptrainpid \
        && echo [train started] \
